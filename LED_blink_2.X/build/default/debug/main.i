...
main:
    ...
    ;--------------------------------------------------------
    ; Configurar ((PORTB) and 0FFh), 0, a como salida para el LED
    ;--------------------------------------------------------
    CLRF LATB, a ; LATB = 0 (apaga todo PORTB para iniciar seguro)
    BCF TRISB, 0, a ; ((TRISB) and 0FFh), 0, a = 0 -> ((PORTB) and 0FFh), 0, a es SALIDA (0 salida, 1 entrada)
...
Blink1s:
    BSF LATB, 0, a ; ((LATB) and 0FFh), 0, a = 1 -> LED ON
    CALL Delay1s ; Esperar 1 segundo
    BCF LATB, 0, a ; ((LATB) and 0FFh), 0, a = 0 -> LED OFF
    CALL Delay1s ; Esperar 1 segundo
...
;============================================================
Delay1s:
    BCF T1CON, 0, a ; bit0 (((T1CON) and 0FFh), 0, a)=0 -> apaga Timer1 para cargar TMR1H/TMR1L
    ...
    BCF PIR1, 0, a ; PIR1 bit0 (((PIR1) and 0FFh), 0, a)=0 -> limpia bandera de overflow de Timer1
    BSF T1CON, 0, a ; ((T1CON) and 0FFh), 0, a=1 -> enciende Timer1 y empieza a contar

;--- Esperar el primer overflow (parcial desde 0x2F70 hasta 0xFFFF) ---
D1_First:
    BTFSS PIR1, 0, a ; ¿((PIR1) and 0FFh), 0, a=1? (si está en 1, hubo overflow)
    BRA D1_First ; Si aún no overflow, seguir esperando (polling)
    BCF PIR1, 0, a ; Limpia ((PIR1) and 0FFh), 0, a para contar el siguiente overflow
...
Delay2s:
    ...
    BCF PIR1, 0, a ; Limpia bandera ((PIR1) and 0FFh), 0, a
    BSF T1CON, 0, a ; Enciende Timer1

;--- Primer overflow parcial ---
D2_First:
    BTFSS PIR1, 0, a ; Esperar ((PIR1) and 0FFh), 0, a=1
    BRA D2_First
